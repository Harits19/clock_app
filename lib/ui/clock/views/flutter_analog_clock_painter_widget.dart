...
class FlutterAnalogClockPainterWidget extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    //clock radius
    final radius = min(size.width, size.height) / 2; //menentukan radius jari2
    //clock circumference
    final double borderWidth =
        showBorder ? (_borderWidth ?? radius / 20.0) : 0.0;
    final double circumference =
        2 * (radius - borderWidth) * pi; // keliling lingkaran

    canvas.translate(size.width / 2, size.height / 2); //mengubah posisi
    ...
    canvas.drawCircle(
        ...
        Paint()
          ...
          ..color = dialPlateColor); // gambar background
    ...
    if (showBorder && borderWidth > 0) {
      Paint borderPaint = Paint()
        ...
        ..isAntiAlias = true;
      canvas.drawCircle(
          Offset(0, 0), radius - borderWidth / 2, borderPaint); // gambar border
    }
